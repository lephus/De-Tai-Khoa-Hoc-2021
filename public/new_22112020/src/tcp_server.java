...
public class tcp_server extends Thread {
	...
	public static String ROOT = "https://gr3atcode.com/";
	...
	private ArrayList<Obj_project_detail> GetProjectDetail(Connection conn) {
		...
		try {
			...
			while (rs.next()) {
				...
				updated = rs.getString("updated");
				is_disabled = rs.getString("is_disabled");
				Obj_project_detail tmp = new Obj_project_detail(id, project_id, content_html, views, updated,
						is_disabled);
	...
	private ArrayList<Obj_hottrend_detail> GetHottrendDetail(Connection conn) {
		...
		try {
			...
			while (rs.next()) {
				...
				updated = rs.getString("updated");
				is_disabled = rs.getString("is_disabled");
				Obj_hottrend_detail tmp = new Obj_hottrend_detail(id, id_hottrend, content_html, views, updated,
						is_disabled);
...
class Process extends Thread {
		...
		try {
			...
			if (action.equals("getDetail")) {
				...
				if (table.equals("course")) {
					String res = process_course_detailpost(action, table, link);
					dos.writeUTF(res);
				} else if(table.equals("project")){
					String res = process_project_detailpost(action, table, link);
					dos.writeUTF(res);
				}
				else if(table.equals("hottrend")){
					String res = process_hottrend_detailpost(action, table, link);
					dos.writeUTF(res);
				}
				else if(table.equals("funny")) {
					String res = process_funny_detailpost(action, table, link);
					dos.writeUTF(res);
				}
				return;
			}
	...
	public String process_vote(String action, String table, String start, String p_id) {
		...
		if (action.equals("getCondition")) {
			...
			String[] atts = new String[] { "id", "subscribe_yt", "member_group", "number_post", "updated",
					"is_disabled" };
			...
			if (p >= n) {
				...
				for (int i = p; i <= k; i++) {
					...
					String[] values = new String[] { temp.id, temp.subscribe_yt, temp.member_group, temp.number_post,
							temp.updated, temp.is_disabled };
		...
		if (action.equals("getCondition")) {
			...
			if (p >= n) {
				...
				for (int i = p; i <= k; i++) {
					...
					String[] values = new String[] { temp.id, temp.id_user, temp.ranking, temp.updated,
							temp.is_disabled };
	...
	public String process_user(String action, String table, String start, String p_id) {
		...
		if (action.equals("getCondition")) {
			...
			String[] atts = new String[] { "id", "username", "pwd", "name", "major", "role_id", "updated",
					"is_disabled" };
			String[] values = new String[atts.length];
			...
			if (p >= n) {
				...
				for (int i = p; i <= k; i++) {
					...
					String[] values = new String[] { temp.id, temp.username, temp.pwd, temp.name, temp.major,
							temp.role_id, temp.updated, temp.is_disabled };
		...
		if (action.equals("getCondition")) {
			...
			if (p >= n) {
				...
				for (int i = p; i <= k; i++) {
					...
					String[] values = new String[] { temp.id, temp.project_id, temp.content_html, temp.views,
							temp.updated, temp.is_disabled };
	...
	public String process_project_detailpost(String action, String table, String link) {
		String []tmp_link = link.split("/");
		ArrayList<Obj_project> lst_data_project = GetprojectAll();
		int n = lst_data_project.size();
		Obj_project t = null;
		for (int i = 0; i < n; i++) {
			if (tmp_link[tmp_link.length - 1].equals(lst_data_project.get(i).link)) {
				t = lst_data_project.get(i);
				break;
			}
		}
		if (t == null) {
			...
			ArrayList<Obj_project_detail> lst_data_project_detail = Getproject_detailAll();
			int nn = lst_data_project_detail.size();
			String project_id = t.id;
			Obj_project_detail t_detail = null;
			for (int i = 0; i < nn; i++) {
				if (lst_data_project_detail.get(i).project_id.equals(project_id)) {
					t_detail = lst_data_project_detail.get(i);
				}
			}
			if (t_detail == null) {
				String res = "{ \"status\": \"Not found\" }";
				...
				String[] atts = new String[] { "status", "title", "content_html"};
				String[] values = new String[atts.length];
				values[0] = "OK";
				values[1] = t.title;
				values[2] = t_detail.content_html;
				String res = Util.createJson(atts, values);
				return res;
			}
		}
		...
		if (action.equals("getCondition")) {
			...
			if (p >= n) {
				...
				for (int i = p; i <= k; i++) {
					...
					String[] values = new String[] { temp.id, temp.id_hottrend, temp.content_html, temp.views,
							temp.updated, temp.is_disabled };
	...
	public String process_hottrend_detailpost(String action, String table, String link) {
		String []tmp_link = link.split("/");
		ArrayList<Obj_hottrend> lst_data_hottrend = GethottrendAll();
		int n = lst_data_hottrend.size();
		Obj_hottrend t = null;
		for (int i = 0; i < n; i++) {
			if (tmp_link[tmp_link.length - 1].equals(lst_data_hottrend.get(i).link)) {
				t = lst_data_hottrend.get(i);
				break;
			}
		}
		if (t == null) {
			...
			ArrayList<Obj_hottrend_detail> lst_data_hottrend_detail = Gethottrend_detailAll();
			int nn = lst_data_hottrend_detail.size();
			String hottrend_id = t.id;
			Obj_hottrend_detail t_detail = null;
			for (int i = 0; i < nn; i++) {
				if (lst_data_hottrend_detail.get(i).id_hottrend.equals(hottrend_id)) {
					t_detail = lst_data_hottrend_detail.get(i);
				}
			}
			if (t_detail == null) {
				String res = "{ \"status\": \"Not found\" }";
		...
		if (action.equals("getCondition")) {
			...
			if (p >= n) {
				...
				for (int i = p; i <= k; i++) {
					...
					String[] values = new String[] { temp.id, temp.title, temp.short_desc, temp.thumbnail, temp.link,
							temp.updated, temp.is_disabled };
		...
		if (action.equals("getCondition")) {
			...
			if (p >= n) {
				...
				for (int i = p; i <= k; i++) {
					...
					String[] values = new String[] { temp.id, temp.id_funny, temp.content_html, temp.views,
							temp.updated, temp.is_disabled };
	 ...
	 */
	public String process_funny_detailpost(String action, String table, String link) {
		String [] tmp_link = link.split("/");
		...
		Obj_funny t = null;
		for (int i = 0; i < n; i++) {
			if (tmp_link[tmp_link.length - 1].equals(lst_data_funny.get(i).link)) {
				t = lst_data_funny.get(i);
				break;
			}
		}
		if (t == null) {
			...
			ArrayList<Obj_funny_detail> lst_data_funny_detail = Getfunny_detailAll();
			int nn = lst_data_funny_detail.size();
			String funny_id = t.id;
			Obj_funny_detail t_detail = null;
			for (int i = 0; i < nn; i++) {
				if (lst_data_funny_detail.get(i).id_funny.equals(funny_id)) {
					t_detail = lst_data_funny_detail.get(i);
				}
			}
			if (t_detail == null) {
				String res = "{ \"status\": \"Not found\" }";
	 ...
	 */
	public String process_course_detailpost(String action, String table, String link) {
		String []tmp_link = link.split("/");
		ArrayList<Obj_course> lst_data_course = GetCourseAll();
		int n = lst_data_course.size();
		Obj_course t = null;
		for (int i = 0; i < n; i++) {
			if (tmp_link[tmp_link.length - 1].equals(lst_data_course.get(i).link)) {
				t = lst_data_course.get(i);
				break;
			}
		}
		if (t == null) {
			...
			int nn = lst_data_course_detail.size();
			String course_id = t.id;
			Obj_course_detail t_detail = null;
			for (int i = 0; i < nn; i++) {
				if (lst_data_course_detail.get(i).course_id.equals(course_id)) {
					t_detail = lst_data_course_detail.get(i);
				}
			}
			if (t_detail == null) {
				String res = "{ \"status\": \"Not found\" }";
		...
		if (action.equals("getCondition")) {
			...
			if (p >= n) {
				...
				for (int i = p; i <= k; i++) {
					...
					String[] values = new String[] { temp.id, temp.course_id, temp.content_html, temp.views,
							temp.updated, temp.is_disabled };
		...
		if (action.equals("getCondition")) {
			...
			if (p >= n) {
				...
				for (int i = p; i <= k; i++) {
					...
					String[] values = new String[] { temp.id, temp.name, temp.email, temp.content, temp.updated,
							temp.is_disabled };
		...
		if (action.equals("getCondition")) {
			...
			if (p >= n) {
				...
				for (int i = p; i <= k; i++) {
					...
					String[] values = new String[] { temp.id, temp.type_comment_id, temp.content, temp.id_post,
							temp.updated, temp.is_disabled };
